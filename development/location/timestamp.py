...
class Timestamper(object):
    ...
    def __call__(self,timestamp_array):
        result = numpy.zeros((len(timestamp_array),self.total_length),dtype=theano.config.floatX)

        for i in xrange(len(timestamp_array)):
            dt = datetime.datetime.utcfromtimestamp(timestamp_array[i])
            j = 0
            self.writeDay(result[i][:self.nodes_per_day],dt)
            j = self.nodes_per_day
            self.writeWeek(result[i][j:j+self.nodes_per_week],dt)
            j += self.nodes_per_week
            self.writeMonth(result[i][j:j+self.nodes_per_month],dt)
            j+= self.nodes_per_month
            self.writeYear(result[i][j:],dt)
        return result
